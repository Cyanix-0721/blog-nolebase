...
## 1 **✅ 方法 1：使用 Hyprland 的 `exec` 或 `exec-once`（推荐）**
...
## 2 **✅ 方法 2：使用 `dex`（Desktop Entry Execution）**
...
| 方法 | 适用场景 | 推荐度 |
|------|----------|--------|
| **`exec-once`（Hyprland 原生）** | 简单命令、Waybar、状态栏工具 | ⭐⭐⭐⭐⭐ |
| **`dex --autostart`** | 兼容 `.desktop` 文件 | ⭐⭐⭐⭐ |
| **`systemd` 用户服务** | 长期运行的后台服务 | ⭐⭐⭐⭐ |
| **`autostart.sh` 脚本** | 需要延迟启动的情况 | ⭐⭐⭐ |

**推荐方案**：
1. **优先使用 `exec-once`**（Hyprland 原生支持）。
2. 如果需要 `.desktop` 文件，用 `dex --autostart`。
3. 如果是守护进程，用 `systemd`。
